...
public class MatrixMult {
    ...
    public static void parallelMatrixMultiply(Matrix a, Matrix b, Matrix c,
        ...
        // Broadcast all matrices to all ranks
        ...
        for (int i = startRow; i < endRow; i++) {
            int off = a.getOffsetOfRow(i);
            for (int j = 0; j < c.getNCols(); j++) {
                int index = c.getOffsetOfRow(i) + j;
                c.getValues()[index] = 0.0;
                for (int k = 0; k < b.getNRows(); k++) {
                    c.incr(i, j, a.getValues()[off + k] * b.get(k, j));
                }
            }
        }
